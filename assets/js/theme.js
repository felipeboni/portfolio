...
(function($) {
    ...
    //====== Get last infos from github
    const user = "felipeboni";
    ...
    $.ajax({
        ...
        response.some((evt, i) => {
            ...
            console.log(evt, i)
            const { repo } = evt;
            const commit = evt.payload.commits[0];
                    ...
                    <div class="col-lg-4 col-md-6 col-sm-12">
                        <div class="blog-post-item mb-40 wow fadeInUp" data-wow-delay=".25s">
                            <div class="entry-content">
                                <a target="blank" class="cat-btn text-white">${moment(evt.created_at).format("MMM Do")}</a>
                                <h3 class="title"><a href="${repo.html_url}" target="blank">${repo.name}</a></h3>
                                <p>${commit.commit.message}</p>
                                <a href="${commit.html_url}" target="blank" class="btn-link">Read More</a>
                            </div>
                        </div>
                    </div>
                    `)
    ...
    //====== Calculate age
    $(".my-birthday").text(moment().diff('2001-09-27', 'years'));
